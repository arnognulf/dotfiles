...
function _YANKY
{
    ...
    echo "${COUNT} files yanked" >&2 | command cat >/dev/null
    else
    ...
    command echo -e "\n${YANKED_LINES} lines yanked" >&2 | command cat >/dev/null
    fi
}
...
"
    ...
    echo -e "\n${COUNT} more files" >&2 | command cat >/dev/null
    else
    local YANKYPANKY_STDIO_FILE=/tmp/yankypanky-${UID}
    ...
    then
        ...
        echo -e "\n${PANKED_LINES} more lines" >&2 | command cat >/dev/null
    else
    wl-paste 2>/dev/null || \
    xclip -o 2>/dev/null
    fi
